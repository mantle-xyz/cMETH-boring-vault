...
contract DeployArcticArchitecture is Script, ContractNames {
    ...
    function _getAddressIfDeployed(
        string memory name
    ...
    function _deploy(
        ...
        if (configureDeployment.deployContracts) {
            ...
            if (deployedAddress == address(0)) {
                ...
                lens = ArcticArchitectureLens(
                    deployer.deployContract(names.lens, creationCode, hex"", 0)
                );
            } else {
            ...
            if (deployedAddress == address(0)) {
                ...
                constructorArgs = abi.encode(
                    owner,
                    address(boringVault),
                    configureDeployment.balancerVault
                );
            ...
            boringVault = BoringVault(
                payable(_getAddressIfDeployed(names.boringVault))
            );
            manager = ManagerWithMerkleVerification(
                _getAddressIfDeployed(names.manager)
            );
            accountant = AccountantWithRateProviders(
                _getAddressIfDeployed(names.accountant)
            );
            teller = TellerWithMultiAssetSupport(
                payable(_getAddressIfDeployed(names.teller))
            );
            rawDataDecoderAndSanitizer = _getAddressIfDeployed(
                names.rawDataDecoderAndSanitizer
            );
            delayedWithdrawer = DelayedWithdraw(
        ...
        if (configureDeployment.setupRoles) {
            ...
            if (
                !rolesAuthority.doesRoleHaveCapability(
                    ...
                    address(boringVault),
                    bytes4(
                        abi.encodeWithSignature("manage(address,bytes,uint256)")
                    )
                )
            ) {
            ...
            if (
                !rolesAuthority.doesRoleHaveCapability(
                    OWNER_ROLE,
                    address(accountant),
                    AccountantWithRateProviders.updateManagementFee.selector
                )
            ) {
                rolesAuthority.setRoleCapability(
                    OWNER_ROLE,
            ...
            if (
                !rolesAuthority.doesRoleHaveCapability(
                    OWNER_ROLE,
                    address(teller),
                    TellerWithMultiAssetSupport.addAsset.selector
                )
            ) {
                rolesAuthority.setRoleCapability(
                    OWNER_ROLE,
            ...
            if (
                !rolesAuthority.doesRoleHaveCapability(
                    OWNER_ROLE,
                    address(teller),
                    TellerWithMultiAssetSupport.removeAsset.selector
                )
            ) {
                rolesAuthority.setRoleCapability(
                    OWNER_ROLE,
        ...
        if (configureDeployment.setupDepositAssets) {
            ...
            for (uint256 i; i < depositAssets.length; i++) {
                ...
                (bool isPeggedToBase, IRateProvider rateProvider) = accountant
                    .rateProviderData(depositAssets[i].asset);
                ...
                if (depositAsset.isPeggedToBase) {
                    ...
                    constructorArgs = abi.encode(
                        depositAsset.target,
                        depositAsset.selector,
                        depositAsset.params
                    );
                    address deployedAddress = _getAddressIfDeployed(
                        depositAsset.genericRateProviderName
                    );
                    if (deployedAddress == address(0)) {
        ...
        if (configureDeployment.setupWithdrawAssets) {
            ...
            for (uint256 i; i < withdrawAssets.length; i++) {
                (bool allowWithdraws, , , , , ) = delayedWithdrawer
                    .withdrawAssets(withdrawAssets[i].asset);
        ...
        if (configureDeployment.finishSetup) {
            ...
            // Setup roles.
            if (
                !rolesAuthority.doesUserHaveRole(address(manager), MANAGER_ROLE)
            ) {
            ...
            if (
                !rolesAuthority.doesUserHaveRole(address(teller), MINTER_ROLE)
            ) {
        ...
        if (configureDeployment.setupTestUser) {
            ...
            if (
                !rolesAuthority.doesUserHaveRole(developmentAddress, OWNER_ROLE)
            ) {
                rolesAuthority.setUserRole(
                    developmentAddress,
                    OWNER_ROLE,
        ...
        if (configureDeployment.saveDeploymentDetails) {
...

            {
                ...
                coreOutput = vm.serializeAddress(
                    coreContracts,
                    "DelayedWithdraw",
                    address(delayedWithdrawer)
                );
            ...
            {
                string
                    memory depositAssetConfiguration = "deposit asset configuration key";
                // Add the base asset.
                string memory assetKey = "asset key 0";
                vm.serializeBool(assetKey, "isPeggedToBase", true);
                string memory assetOutput = vm.serializeAddress(
                    assetKey,
                    "rateProvider",
                    address(0)
                );
                depositAssetConfigurationOutput = vm.serializeString(
                    ...
                    accountantParameters.base.symbol(),
                    assetOutput
                );
                for (uint256 i; i < depositAssets.length; i++) {
                    ...
                    vm.serializeBool(
                        assetKey,
                        "isPeggedToBase",
                        depositAsset.isPeggedToBase
                    );
                    assetOutput = vm.serializeAddress(
                        assetKey,
                        "rateProvider",
                        depositAsset.rateProvider
                    );
            ...
            {
                string
                    memory withdrawAssetConfiguration = "withdraw asset configuration key";
                for (uint256 i; i < withdrawAssets.length; i++) {
                    WithdrawAsset memory withdrawAsset = withdrawAssets[i];
                    ...
                    string memory assetOutput = vm.serializeUint(
                        assetKey,
                        "MaxLoss",
                        withdrawAsset.maxLoss
                    );
                    withdrawAssetConfigurationOutput = vm.serializeString(
                        withdrawAssetConfiguration,
                        withdrawAsset.asset.symbol(),
                        assetOutput
                    );
                }
            }
            {
                ...
                vm.serializeAddress(
                    accountantConfiguration,
                    "BaseAddress",
                    address(accountantParameters.base)
                );
                accountantConfigurationOutput = vm.serializeString(
                    accountantConfiguration,
                    "Base",
                    accountantParameters.base.symbol()
                );
            ...
            {
                string
                    memory depositConfiguration = "deposit configuration key";
                vm.serializeBool(
                ...
                depositConfigurationOutput = vm.serializeUint(
                    depositConfiguration,
                    "ShareLockPeriod",
                    shareLockPeriod
                );
