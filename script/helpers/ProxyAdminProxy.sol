...
struct L1ProxyAdminDeployments {
...
struct L2ProxyAdminDeployments {
    TimelockController proxyAdmin;
    L2cmETH l2cmETH;
...
struct L1ProxyAdminDeploymentParams {
    address admin;
    address owner;
    address delegate;
    address upgrader;
...
struct L2ProxyAdminDeploymentParams {
    address admin;
    address owner;
    address delegate;
    address upgrader;
...
function deployL1(L1ProxyAdminDeploymentParams memory params) returns (L1ProxyAdminDeployments memory) {
    return deployL1(params, msg.sender);
}

function deployL2(L2ProxyAdminDeploymentParams memory params) returns (L2ProxyAdminDeployments memory) {
...
function deployL1(L1ProxyAdminDeploymentParams memory params, address deployer) returns (L1ProxyAdminDeployments memory) {
    ...
    L1ProxyAdminDeployments memory ds = L1ProxyAdminDeployments({
...
/// @param deployer the address executing this function. While this will always be `msg.sender` in deployement scripts,
/// it will need to be set in tests as `prank`s will not affect `msg.sender` in free functions.
function deployL2(L2ProxyAdminDeploymentParams memory params, address deployer) returns (L2ProxyAdminDeployments memory) {
    address[] memory controllers = new address[](2);
    controllers[0] = params.upgrader;
    controllers[1] = deployer;
    TimelockController proxyAdmin =
    ...
    EmptyContract empty = new EmptyContract();
    // Create proxies for all contracts
    L2ProxyAdminDeployments memory ds = L2ProxyAdminDeployments({
        proxyAdmin: proxyAdmin,
